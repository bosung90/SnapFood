...
namespace SnapFood
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private async Task UploadImage(StorageFile file)
        {
            StorageCredentials sc = new StorageCredentials("portalvhdsq9mb4wm4df10b",
                "dMI5pbF35foV9ZUCWbLCrADmOpr3uZiJh8w+pLkSFhFY6AjFGFdyMEAaYdDAScIp+bRzZpa/Rci94RFubi21Jw==");

            CloudStorageAccount storageAccount = new CloudStorageAccount(sc, false);

            CloudBlobClient blobClient = storageAccount.CreateCloudBlobClient();

            CloudBlobContainer container = blobClient.GetContainerReference("snapfood");

            CloudBlockBlob blockBlob = container.GetBlockBlobReference(file.Name);

            var uploaded = blockBlob.UploadFromFileAsync(file);
